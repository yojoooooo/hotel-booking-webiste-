...
const Departures: React.FC = () => {
  const [departures, setDepartures] = useState<(BookingType & { hotelName: string })[]>([]);
  ...
  const fetchDepartures = async () => {
    try {
      const today = dayjs();
      const bookings = await apiClient.fetchBookingsByCheckoutDate(today);
      ...
      setDepartures(bookingsWithHotelNames);
    } catch (error: any) {
      setError(error.message || "Error fetching departures.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h2 className="text-3xl font-bold mb-8 text-gray-700 text-center">Departures Today</h2>

      {departures.length === 0 ? (
        <div className="text-center text-lg text-gray-500">
          No departures available today.
        </div>
      ) : (
